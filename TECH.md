...
- **Command rail** – Right-aligned header exposing Preview, Generator, Fullscreen, Import, Saves, Help, and Settings buttons through icon-only controls. Preview reveals the clustered artwork, Fullscreen pushes the stage edge-to-edge, Import accepts images or JSON puzzles, and the Generator/Saves/Help icons deep-link into the Settings & tools slide-over on their respective tabs.
