...
- **Puzzle rendering pipeline.** `renderPuzzle` composites the stored base image first, then fills an offscreen mask covering every unfinished region so the original art shows through as you paint. The mask rasterizes into the cached `filledLayer`, while the WebGL renderer now streams region contours to the GPU as line-strip buffers and draws them directly instead of uploading a raster outline layer. Labels use the cached geometry as well: we flatten glyph layout per region, feed it into a glyph atlas texture (stroke lives in the red channel, fill in green), and draw quads in a single pass so colours stay dynamic and the overlay remains crisp at any zoom. Label layout caches include a rounded zoom key derived from the current display-to-base scale ratio, shrink the minimum font size relative to that zoom factor, and expand their attempt list at higher zoom so thin regions surface labels once there is enough on-screen room without sacrificing base zoom legibility. Visual feedback continues to leverage `flashColorRegions` and `paintRegions` for hints. The renderer controller proxies those calls to Canvas2D, WebGL, or SVG backends—WebGL still uploads the base and fill layers as textures, tracks incremental fills so textures refresh immediately, and preserves the last good upload when a transfer fails so the screen never flashes blank, while the SVG renderer mounts the base image under its region mask before emitting `<path>` nodes so vector output stays crisp at any zoom.
