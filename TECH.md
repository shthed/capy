...
- **Puzzle rendering pipeline.** `renderPuzzle` still composites the stored base image first, then fills an offscreen mask covering every unfinished region so the original art shows through as you paint. The WebGL backend now renders from a viewport-anchored camera descriptor (centre, pixels-per-unit, viewport width/height) instead of relying on DOM scaling, so outlines and labels remain crisp while you zoom indefinitely. Region contours stream to the GPU as line-strip buffers, filled regions upload as textures, and label quads come from the cached glyph atlas in a single pass. Canvas2D and SVG remain as fallbacks behind the renderer controller for environments without WebGL or infinite-zoom support.
- **Procedural world streaming.** `render.js` owns `createProceduralWorldManager`, which tracks the root puzzle geometry plus higher-LOD tiles returned by `hooks.requestProceduralTile({ level, x, y, view })`. The manager selects the highest ready tile that covers the active camera, exposes `getActiveGeometry()` for the renderer, and surfaces `getTiles()` metadata (including an `isActive` flag) for diagnostics. Renderer capabilities flow back through `rendererController.getCapabilities()`/`.capabilities` so the UI can disable legacy pan clamping when `{ infiniteWorld: true, viewportAnchored: true }` comes from WebGL, while Canvas2D/SVG report false and continue using DOM transforms.
...
### Infinite Zoom & Procedural Tiles

- The viewport now tracks both pixel-space pan offsets and puzzle-space centres. Use `setViewPan`, `adjustViewPan`, or `setViewCenter` inside `index.html` when mutating the view so `viewState.centerX/centerY` stay synchronised with `panX/panY` and zoom math.
- `getViewCamera()` derives the renderer-facing camera descriptor—centre coordinates, `pixelsPerUnit`, device pixel ratio, and viewport size—and feeds it into `renderFrame`/`flashRegions`. WebGL resolves `pixelsPerUnit` from `metrics.renderScale`, while Canvas/SVG fall back to DOM scaling when they report `viewportAnchored: false`.
- Renderer capabilities are exposed via `rendererController.capabilities`/`getCapabilities()`. WebGL reports `{ infiniteWorld: true, viewportAnchored: true, minZoom, maxZoom, supportsProceduralTiles }`, enabling the UI to disable legacy pan clamping and preserve viewport sizing. Canvas2D/SVG return `{ infiniteWorld: false, viewportAnchored: false }` so the old pixel-locked behaviour continues.
- When provided, `hooks.requestProceduralTile({ level, x, y, view })` should resolve with `{ regions, bounds }`, where `regions` is an array of GPU-ready geometries for the requested tile and `bounds` describes its width/height in puzzle units. `createProceduralWorldManager` requests finer tiles as `pixelsPerUnit` crosses powers of two (512, 1024, …), falls back to coarser geometry until a tile is ready, and marks the active tile via `getTiles()` metadata for diagnostics.
- `rendererController.getWorld()` returns the procedural manager so future AI generators can inspect cached tiles, kick off background requests, or surface debugging overlays without touching renderer internals.
