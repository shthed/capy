...
- **Pan/zoom subsystem.** `viewState` tracks CSS transforms for `#canvasStage` and `#canvasTransform`: `applyViewTransform` writes `--pan-x`/`--pan-y` for translation and `--zoom` for scale while `applyCanvasSizing` keeps the backing canvas at a fixed native resolution (puzzle dimensions × device pixel ratio, floored to at least 1×). Gestures therefore only mutate CSS, and we re-render once per interaction (or when the base fit scale/device pixel ratio changes) so zooming stays GPU-composited and buttery smooth.
