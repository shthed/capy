...
- `loadPuzzleFixture(puzzle)`
  - **Parameters:** Fixture payload or identifier.
  - **Purpose:** Loads one of the bundled sample puzzles.
  - **Side effects:** Resets the UI, applies the puzzle result, hides the start screen, and returns whether hydration succeeded.
...
- `togglePreview(show)`
  ...
  - **Side effects:** Updates preview state, persists the visibility flag, and refreshes UI affordances.
