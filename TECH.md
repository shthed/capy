...
- **Testing & QA**
  - `project/tests/ui-review.spec.js` – Playwright smoke suite that boots Chromium, seeds an automation puzzle, and covers renderer toggles, palette counters, and save/load flows via the shared fixtures.
    ...
    - Captures runtime performance metrics via `window.capyGenerator.getPerformanceMetrics()` so hydration, render, and fill timings are validated alongside the visual assertions.
    - Resets metrics mid-test through `window.capyGenerator.resetPerformanceMetrics()` to validate hydration and interaction measurements independently.
...
- **Public testing surface.** `window.capyGenerator` exposes helpers (`loadFromDataUrl`, `loadPuzzleFixture`, `togglePreview`, etc.) so automation and manual experiments can orchestrate the app without touching internals. Recent renderer work also surfaced `getRendererType()`, `listRenderers()`, `setRenderer(type)`, `registerRenderer(type, factory)`, and `unregisterRenderer(type)` so tests can assert the active backend or load experimental renderers without patching private state. Performance helpers – `getPerformanceMetrics()` and `logPerformanceMetrics(reason)` – mirror the runtime metrics collector so investigations can sample aggregated timings on demand.
- **Performance helpers.** Use `resetPerformanceMetrics()` when you need a clean slate before replaying a scenario so follow-up measurements only include the interactions under test.
...
- **Performance instrumentation.** `createPerformanceMetrics` wraps bootstrap, puzzle hydration, renderer frames, and fill interactions with timestamped measurements. Summaries flush to the browser console every few seconds (and on-demand via `window.capyGenerator.logPerformanceMetrics(reason)`) so responsiveness regressions stand out while iterating. Aggregated snapshots – exposed through `getPerformanceMetrics()` – track counts, averages, peaks, and recent samples for automation and manual profiling alike.
