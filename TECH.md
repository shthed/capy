...
- **Detailed debug logging.** The Help panel’s live log announces sample loads, fills, hints, zooms, background tweaks, fullscreen toggles, ignored clicks, and now mirrors console warnings/errors (including unhandled rejections) so QA can confirm issues without opening DevTools.
...
4. **Fill regions.** Pick a colour and click/tap cells. Nearby difficulty lets you jump to adjacent colours without changing your active selection (colour 2 can also paint 1 or 3), while Easy auto-selects any tapped colour before painting. Auto-advance hops to the next incomplete colour when enabled.
