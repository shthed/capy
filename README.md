...
- **Puzzle rendering pipeline.** `renderPuzzle` orchestrates drawing the current
  canvas via the Path2D-backed geometry cache generated by `ensureRenderCache`,
  then calls `drawOutlines` and `drawNumbers`. Visual feedback uses
  `flashColorRegions` and `paintRegions` to overlay tint pulses without
  rebuilding pixel masks.
...
- **Region clicks.** The canvas click handler first validates that a puzzle and
  ...
  required, while successful fills add the region to the completed set and
  invoke `renderPuzzle` to redraw the vector layers.
