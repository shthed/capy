...
- Sync the working tree before writing code with `npm run git:update`, which
  runs `git fetch --all --prune` and then `git status -sb` to highlight
  unexpected changes or conflicts (run the two commands manually if you prefer
  not to use the helper script).
