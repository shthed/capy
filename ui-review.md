...
- The new generator status tray in the palette dock shows a live progress bar, a telemetry grid (mode/prompt, source & target sizes, palette/region totals, background), plus fading notifications for file reads, k-means clustering, smoothing, segmentation counts, and palette prep, making it obvious when the import pipeline is working or blocked.
