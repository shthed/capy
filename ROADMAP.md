...
## Now (stabilise)
- Bring CI back to the full Node + Playwright harness and restore hosted smoke checks so deployments regain automatic coverage.
- Harden offline support by adding cache caps/eviction to the service worker before introducing larger assets or remote fetches.
- Keep handbook docs (`TECH.md`, `AGENTS.md`, `STYLEGUIDE.md`) aligned with runtime behaviour so contributors land on accurate guidance.
