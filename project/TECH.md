...
- **Pan/zoom subsystem.** `viewState` still records the aggregate translate/scale that `applyZoom`, `handleMouseDragZoom`, `resetView`, and `applyViewTransform` consume, but the long-term target is an infinite zoom rail that never clamps to `DEFAULT_MAX_VIEWPORT_ZOOM` (currently 12×). Start by virtualising the scale value: convert the persisted `state.settings.maxZoom` knob into a soft guard, but let the interaction helpers keep stepping beyond 1200 % while emitting a tile level such as `viewState.detailLevel`. That level feeds the renderer cache so we can request deeper tiles without fighting CSS transforms. As we drop the clamp, keep `resolveMaxZoomCandidate`, `applyZoom`, and the drag handler in sync so they normalise gesture deltas into logarithmic scale steps (preventing float drift while you continue zooming into microscopic regions). The virtual origin for panning should stay centred on `#canvasStage` so new tiles always expand from the viewport centre instead of the original bitmap’s bounds.
...
- **Puzzle rendering pipeline.** `renderPuzzle` currently paints palette-coloured vector geometry directly (cached region paths fill with their target colours, unfinished areas inherit the configurable background tint, outlines reuse the stroke cache, and `drawNumbers` renders label text from shared placement data instead of raster thumbnails). To reach the infinite zoom goal, treat those caches as tile streams instead of single-scene blobs: extend `createRenderCache` and the renderer hooks so each zoom level requests geometry in discrete `detailLevel` buckets (for example, base scene, +2×, +4×) and keeps only the tiles intersecting the current viewport. The SVG renderer (`createSvgRenderer`) already keeps a DOM node per region, so swap the one-time `rebuildGeometry` with a diff that adds/removes tiles as zoom increases. Canvas2D/WebGL should mirror that behaviour by drawing into offscreen canvases keyed by `{tileId, level}` and compositing them each frame. Every renderer continues to record frame timing, but now they must also surface tile-miss metrics so `logRendererEvent` can alert when the viewport outruns queued tiles. Label placement needs the same treatment—`computeLabelPlacements` should request new placements from the generator or AI backend when the tile density exceeds the stored centroid spacing, rather than relying on a single global cache that was computed at load time.

- **AI-guided region expansion.** Infinite zoom hinges on generating fresh regions as the player dives deeper. `puzzle-generation.js` already exposes `createPuzzleData`, `normalizePuzzleOptions`, and the `GENERATION_ALGORITHM_CATALOG`; piggy-back on those entry points so renderers can ask for “deeper” tiles. At a high level: when the renderer reports a tile miss, enqueue an async request through a new `requestDeeperRegions({ tileId, level, promptContext })` helper inside `puzzle-generation.js`. That helper either calls into an AI service (fed by the stored `state.settings.artPrompt`/`imageDescription`) or reruns the local segmentation pipeline against a supersampled crop. Persist each response under a new `deepZoomTiles` map in the save payload so reloads do not lose generated content. `render.js` consumes those updates through its existing `renderCache` structure: when `renderPuzzle` observes new tiles for a colour, it merges them into the in-memory geometry map and schedules a renderer refresh. Keep hooks symmetrical—the Canvas2D fallback should ignore curve metadata it cannot replay, but the SVG path builder must preserve full curve instructions so AI services can return cubic and quadratic segments without downgrading to line soup.

- **Streaming + storage considerations.** Infinite zoom tiles will explode memory usage if we never evict them. Track the current viewport’s bounding box inside `viewState` (or a sibling `tileWindow` object) and expose a `trimDeepTiles({ maxTiles, maxBytes })` helper that walks `renderCache.deepZoomTiles` and evicts tiles furthest from the viewport centre once they fall outside the last N zoom levels. Persist eviction stats through `performanceMetrics` so QA can confirm we are not leaking DOM nodes. Label placement fallbacks should downgrade to canvas text overlays when `getLabelPlacements` cannot keep up—the SVG backend can temporarily hide number glyphs for empty tiles, but it should announce the gap via `logRendererEvent` so we can chase generator latency regressions.
