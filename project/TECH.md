...
- **Puzzle rendering pipeline.** `renderPuzzle` now paints palette-coloured vector geometry directly: cached region paths fill with their target colours, unfinished areas inherit the configurable background tint, outlines reuse the stroke cache, and `drawNumbers` renders label text from shared placement data instead of raster thumbnails so glyphs stay sharp at any zoom. Label layout caches still hinge on the display-to-base scale ratio and widen their search at higher zoom to keep tiny regions legible without shrinking the base view. Visual feedback continues to leverage `flashColorRegions` and `paintRegions` for hints. The renderer controller proxies those calls to Canvas2D, WebGL, or SVG backends—the default SVG path draws fills, outlines, and label text as scalable vectors, WebGL uploads the cached layers as textures while preserving the last good transfer on error, and Canvas2D remains a compatibility fallback. Each renderer samples its frame times and prints rolling averages to the browser console once per second so you can spot performance regressions while debugging pan and zoom flows.
