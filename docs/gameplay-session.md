...
- Palette selection immediately highlighted the active swatch, flashed every matching region (with a celebratory flash once a colour is complete), and the new palette controls let me drag-scroll through large palettes, auto-hide finished colours, and flip between numeric, remaining, or hue-based sort orders while keeping numbers crisp and counts surfaced via tooltip copy.
